...
- You can also add editor-specific styles for adjusting the DOM elements added by the editor or the editor UI itself (for specific fixes), those editor styles, even they are named like this here, wouldn't be added as editor style (as it is called in WordPress), but rather just enqueued on the admin/backend editor pages as normal styles (which wouldn't be post-processed and hence isolated by Gutenberg editor either).
